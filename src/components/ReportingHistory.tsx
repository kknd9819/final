...
import { Calendar, MapPin, Receipt, Trash2, Award, ClipboardCheck, RefreshCw, Loader2 } from 'lucide-react';
...
export default function ReportingHistory({ submissions, onDelete, onClose, loading = false, onRefresh }: ReportingHistoryProps) {
  return (
    <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-sm rounded-2xl p-6 text-gray-900 dark:text-gray-100 max-w-2xl mx-auto my-6">
      <div className="flex justify-between items-center border-b border-gray-100 dark:border-gray-700 pb-4 mb-6">
        <h3 className="text-xl font-semibold text-gray-900 dark:text-gray-100 flex items-center gap-2">
          ...
          我的提报记录
          {loading && <Loader2 className="w-4 h-4 animate-spin text-blue-500 ml-2" />}
        </h3>
        <div className="flex items-center gap-2">
          {onRefresh && (
            <button
              ...
              disabled={loading}
              type="button"
              className="text-sm font-medium text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 bg-white dark:bg-gray-700 border border-blue-200 dark:border-blue-700 hover:bg-blue-50 dark:hover:bg-gray-600 px-3 py-2 rounded-lg transition-colors flex items-center gap-1.5 disabled:opacity-50"
            >
                  ...
                  <div className="flex justify-between items-start gap-4 mb-3">
                    ...
                    <div>
                      {submission.status === 'awarded' ? (
                        ...
                        <span className="text-xs text-blue-700 dark:text-blue-300 bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-800 px-3 py-1.5 rounded-full flex items-center gap-1.5 font-medium">
                          <ClipboardCheck className="w-4 h-4" />
                          已处理
                        </span>
                      ) : submission.status === 'invalid' ? (
                        <span className="text-xs text-red-700 dark:text-red-300 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 px-3 py-1.5 rounded-full flex items-center gap-1.5 font-medium">
                          无效记录
                        </span>
                      ) : (
