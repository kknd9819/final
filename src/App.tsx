...
// 后端 API 地址配置 - 使用自定义域名（国内可访问）
// 需要在 Cloudflare Worker 的 Triggers → Custom Domains 中添加 api.quruifps.xyz
const API_BASE = 'https://api.quruifps.xyz';
...
export default function App() {
  ...
  const handleImageUpload = (id: number | 'others', files: FileList | null) => {
    if (!files) return;
    
    const MAX_TOTAL_BYTES = 10 * 1024 * 1024; // 3MB
  ...
  const handleConfirmSubmit = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        photos: [] as { id: string; name: string; url: string; size: string }[],
        othersPhotos: state.othersPhotos,
      };
      ...
      (Object.entries(state.hazards) as [string, { checked: boolean; photos: { id: string; name: string; url: string; size: string }[] }][]).forEach(([key, val]) => {
        if (val.checked) {
          payload.photos.push(...val.photos);
        }
      });
                    ...
                    >
                      <div className="p-5 flex items-start gap-4">
                        <button
                          ...
                          className={`w-6 h-6 mt-0.5 rounded-lg border flex items-center justify-center flex-shrink-0 transition-colors ${
                            questionState.checked 
